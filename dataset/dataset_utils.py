...
def train_test_split(record_num, split_param, mode='ratio', shuffle=False, category=None):
    '''
    :param record_num:
    ...
    if category is not None:
        ...
        for c in category_idx_dict:
            ...
            sub_train_index, sub_valid_index, sub_test_index = arr_split(
                index_arr, split_param=split_param, mode=mode, shuffle=shuffle)
...
def arr_split(raw_arr, split_param, mode='ratio', shuffle=False):
    '''
...
if __name__ == '__main__':
    c = ['a','a','b','b','a','b','b','a','b','b','c','c','a','c','c','a','c','a','b','b']
    # print(len(c))
    import random
    random.seed(0)
    np.random.seed(0)

    print(train_test_split(20, [0.6,0.2,0.2], category=c))
