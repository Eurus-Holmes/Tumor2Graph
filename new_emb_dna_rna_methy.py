...
#%% load data
file_path = "save_embedding/vgcn_trainer_v2_seed=5_batch=1024_embedding.npy"
...
df_all = pd.read_csv("dataset/v10/df_all_image.csv")
...
df_emb = df_emb_ori[[c for c in df_emb_ori.columns if c not in ["sample","cancer_type"]]]
print(df_emb.shape)
...
#    n_clstrs = cluster_num[cancre_type_]
    print(cancre_type_)
    print('num: %d' % (df_y['true_label']==cancre_type_).sum())
...
noCoverWrite(df_y,'output/hierachical_separate_clstr_autoDecide.csv')
sys.exit()
