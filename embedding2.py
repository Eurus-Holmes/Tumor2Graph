...
file_path = "save_embedding_new_new/vgcn_trainer_v2_seed=5_batch=1024_train_embedding.npy"
...
df_emb_scaled = df_emb.applymap(lambda x: np.exp(x))
# df_emb_scaled = pd.DataFrame(scale(df_emb), index=df_emb.index, columns=df_emb.columns)
...
noCoverWrite(df_y, 'output/dna_rna_methy_cluster_result_new01311.csv')
sys.exit()
