...
df_p = pd.DataFrame()
pdff=PdfPages(f"../data/cluster_result_pfs.pdf")
...
#plt.close()
    ...
    for gp in df.groupby("true_label"):
        ...
        if gp[0]=="KICH":
            continue
        result = multivariate_logrank_test(df_cancer["PFI.time"],df_cancer[cluster_col],df_cancer["PFI"])
        
        cph = CoxPHFitter()
        
        cph.fit(df_cancer[[cluster_col,"PFI.time","PFI"]],duration_col="PFI.time",event_col="PFI")
        #cph.print_summary()
...
#        plt.clf()
    ...
    #    fig,ax = plt.subplot(111)
        ...
        for gp in df_cancer.groupby(cluster_col):
            ...
            kmf.fit(gp[1]["PFI.time"],event_observed = gp[1]["PFI"])
            kmfl.append(kmf)
...
#    plt.close()
    ...
    pdff.savefig()
#pdff.savefig()
plt.savefig(f"../data/{cluster_col}_pfs.png")
